...
Warlock.js provides a powerful scaffolding system through the `create` command family. These commands help you quickly generate boilerplate code for various application components while maintaining consistency and following best practices.

## Overview

The create command system generates:
...
```bash
warlock create.module <name> [options]
```
...
```bash
warlock create.module products
```
...
2. Create controllers using `create.controller`
3. Create models using `create.model`
...
```bash
warlock create.controller <module>/<name> [options]
```
...
```bash
warlock create.controller products/list-products
```
...
```bash
warlock create.controller products/create-product --with-validation
```
...
```bash
warlock create.service <module>/<name> [options]
```
...
```bash
warlock create.service products/create-product
```
...
```bash
warlock create.model <module>/<name> [options]
```
...
```bash
warlock create.model products/product
```
...
```bash
warlock create.model products/product --with-resource
```
...
```bash
warlock create.model products/product --table products_catalog
```
...
```bash
warlock create.repository <module>/<name> [options]
```
...
```bash
warlock create.repository products/product
```
...
```bash
warlock create.resource <module>/<name> [options]
```
...
```bash
warlock create.resource products/product
```
...
```bash
warlock create.validation <module>/<name> [options]
```
...
```bash
warlock create.validation products/update-product
```
...
```bash
warlock create.validation products/update-product --with-request
```
...
All create commands automatically handle name transformations:
...
warlock create.controller products/create-product --with-validation
warlock create.controller products/update-product --with-validation
warlock create.controller products/list-products
warlock create.controller products/get-product

# 5. Create services
warlock create.service products/create-product
warlock create.service products/update-product
warlock create.service products/list-products
warlock create.service products/get-product
```
...
warlock create.controller users/change-password --with-validation

# 2. Create service
warlock create.service users/change-password
```
...
warlock create.validation users/update-profile --with-request
```
...
```bash
$ warlock create.controller products/create-product
Error: Module "products" does not exist
Run: warlock create.module products
```

**Solution**: Create the module first using `create.module`
...
```bash
$ warlock create.controller create-product
Error: Module name is required
Usage: warlock create.controller <module>/<name>
```

**Solution**: Include module name in the path (e.g., `products/create-product`)
...
```bash
# Instead of --with-validation
warlock create.controller products/create-product -v

# Instead of --force
warlock create.model products/product -f
```

### 2. Combine Options

```bash
# Create model with resource and custom table name
warlock create.model products/product --with-resource --table product_catalog
```

### 3. Generate Multiple Components at Once

```bash
# Use shell scripting for batch generation
for action in create update delete list get; do
  warlock create.controller products/${action}-product -v
  warlock create.service products/${action}-product
done
```

### 4. Verify Before Overwriting

Always review generated code before using `--force` to avoid losing custom changes.

---

## See Also

- [Module Structure Guide](../guides/module-structure.md)
- [Validation Documentation](../validation/overview.md)
- [Repository Pattern](../database/repositories.md)
- [CLI Commands Overview](./overview.md)
