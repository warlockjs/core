...
`@warlock.js/web`'s page pipeline is designed to report through this same
`onPhase` surface rather than defining its own hook API (one surface, per the
71622e4a lead decision) — planned phase names are `loader` (once per
app/layout/page level, `attrs: { level, layoutPath }`), `render.shell`, and
`stream.end`. **These are not wired yet** — the design note
(`releases/v5.12-tracing-design-note.md` §6) explicitly defers the `web` half
past 5.12 because `execute-page-request.ts` / `create-page-route-handler.ts`
are mid-refactor for streaming stage 2 in the same release. Only the five
`core` phases above are live today; treat the `web` names as the documented
target shape for when that wiring lands, not as something you can register a
hook against yet.
