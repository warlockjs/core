import { BenchmarkOptions } from "../benchmark/types.mjs";
import { Infer, ObjectValidator } from "@warlock.js/seal";
import { RetryOptions } from "@mongez/reinforcements";

//#region ../../@warlock.js/core/src/use-cases/types.d.ts
/**
 ...
 */
type UseCaseGuard<Input, Ctx extends UseCaseContext = UseCaseContext> = (data: Readonly<Input>, ctx: Ctx) => void | Promise<void>;
/**
 ...
 * @template Ctx - The shape of the shared context
 */
type UseCaseBeforeMiddleware<Input, Ctx extends UseCaseContext = UseCaseContext> = (data: Input, ctx: Ctx) => Input | Promise<Input>;
/**
 ...
 * @template Ctx - The shape of the shared context
 */
type UseCaseAfterMiddleware<Output, Ctx extends UseCaseContext = UseCaseContext> = (output: Output, ctx: Ctx) => void | Promise<void>;
/**
...
type UseCaseOnExecutingContext = {
  /** The shared pipeline context */ctx: UseCaseContext; /** Unique execution ID */
  id: string; /** Use case name */
  name: string; /** Raw input data */
  data: any; /** Schema validator (if defined) */
  schema?: ObjectValidator; /** Timestamp when execution started */
  startedAt: Date;
};
/**
...
type UseCaseBroadcastEvent = {
  /** Use case name */useCase: string; /** Channel/event name — defaults to the use case name */
  event: string; /** Execution id (correlation / idempotency) */
  id: string; /** Emission timestamp */
  at: Date; /** The output as-is, or the projected shape from `broadcast.output` */
  payload: unknown;
};
/**
 ...
 * ```
 */
interface UseCaseBroadcastChannel {
 ...
 */
type UseCaseBroadcastOption<Output> = boolean | {
  /** Channel/event name; defaults to the use case name */event?: string; /** Project the payload before broadcasting (e.g. strip sensitive fields) */
 ...
 * @template Ctx - The shape of the shared context
 */
type UseCase<Output = any, Input = any, Ctx extends UseCaseContext = UseCaseContext> = {
  /** Unique use case identifier, used for registration, logging, and cache keys */name: string; /** Human-readable description, surfaced in the registry and observability */
  description?: string; /** Core business logic handler. Receives validated + transformed data and context */
  handler: (data: Input, ctx: Ctx) => Promise<Output>; /** Optional schema validator (from @warlock.js/seal). Runs after guards */
  schema?: ObjectValidator; /** Guards to run before validation. Sequential, can enrich ctx, cannot mutate data */
  guards?: UseCaseGuard<Input, Ctx>[]; /** Before middleware to run after validation. Sequential, can transform data */
  before?: UseCaseBeforeMiddleware<Input, Ctx>[]; /** After middleware to run on success. Fire-and-forget, errors are logged not thrown */
  after?: UseCaseAfterMiddleware<Output, Ctx>[]; /** Lifecycle callback: fires when execution starts (before guards) */
  onExecuting?: (ctx: UseCaseOnExecutingContext) => void; /** Lifecycle callback: fires on successful completion with full result snapshot */
  onCompleted?: (result: UseCaseResult<Output>) => void; /** Lifecycle callback: fires on error with error details and execution context */
 ...
 * @template Ctx - The shape of the shared context
 */
type UseCaseWithSchema<Output, Schema extends ObjectValidator, Ctx extends UseCaseContext = UseCaseContext> = Omit<UseCase<Output, Infer<Schema>, Ctx>, "schema"> & {
  schema: Schema;
};
/**
 ...
 */
type UseCaseHandler<Output, Input> = ((data: Input, options?: UseCaseRuntimeOptions<Output>) => Promise<Output>) & {
  $cleanup: () => void;
};
/**
 ...
 */
type RegisteredUseCase<Output = any, Input = any> = UseCase<Output, Input> & {
  /** Execution call counters */calls: {
    /** Number of successful executions */success: number; /** Number of failed executions */
    failed: number; /** Total executions (success + failed) */
    total: number;
  };
};
/**
...
type UseCaseRuntimeOptions<Output = any> = {
  /** Override the auto-generated execution ID */id?: string; /** Provide a pre-populated context object */
  ctx?: UseCaseContext; /** Invocation-level lifecycle callback: fires first, before use case and global */
  onExecuting?: (ctx: UseCaseOnExecutingContext) => void; /** Invocation-level lifecycle callback: fires first on success */
  onCompleted?: (result: UseCaseResult<Output>) => void; /** Invocation-level lifecycle callback: fires first on error */
  onError?: (ctx: UseCaseErrorResult) => void;
};
/**
...
type UseCaseErrorResult = Omit<UseCaseResult, "output"> & {
  /** The error that caused the failure */error: Error;
};
/**
 ...
 * @template Output - The shape of the handler's return value
 */
type UseCaseResult<Output = any> = {
  /** Handler return value (undefined if execution failed) */output?: Output; /** The pipeline context at time of completion */
  ctx: UseCaseContext; /** Timestamp when execution started */
  startedAt: Date; /** Timestamp when execution ended */
  endedAt: Date; /** Unique execution ID */
  id: string; /** Use case name */
  name: string; /** Success or failed call count at time of completion (matches the outcome) */
  calls: number; /** Retry state (present only if retry was configured) */
  retries?: {
    /** Total allowed attempts (initial try + retries) */attempts: number; /** Delay between attempts in ms */
    delay?: number; /** Actual retry attempts performed (0 = succeeded on first try) */
    currentRetry: number;
  }; /** Benchmark result (present only if benchmark was enabled) */
  benchmarkResult?: {
    /** Execution time in milliseconds */latency: number; /** Performance classification */
 ...
 */
type UseCaseEventsCallbacksMap = {
...
type UseCaseConfigurations = {
   ...
   */
  benchmark?: boolean | BenchmarkOptions; /** Broadcast transport — registered channels plus a global kill-switch */
  broadcast?: {
    /** Global on/off — `false` disables broadcasting for every use case */enabled?: boolean; /** Channels every broadcast fans out to */
    channels?: UseCaseBroadcastChannel[];
  }; /** Per-step debug logging through `@warlock.js/logger` (default: false) */
  log?: boolean; /** Execution history cache settings */
  history?: {
    /** Enable/disable history storage (default: true) */enabled?: boolean; /** Cache TTL in seconds (default: 3600). `false` uses the cache driver default */
    ttl?: number | false; /** Max history entries kept per use case before oldest are evicted (default: 100) */
    maxEntries?: number;
  };
};
//#endregion
export { RegisteredUseCase, UseCase, UseCaseAfterMiddleware, UseCaseBeforeMiddleware, UseCaseBroadcastChannel, UseCaseBroadcastEvent, UseCaseBroadcastOption, UseCaseConfigurations, UseCaseContext, UseCaseErrorResult, UseCaseEventsCallbacksMap, UseCaseGuard, UseCaseHandler, UseCaseOnExecutingContext, UseCaseResult, UseCaseRuntimeOptions, UseCaseWithSchema };
