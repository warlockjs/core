...
//#region ../../@warlock.js/core/src/storage/scoped-storage.ts
/**
...
var ScopedStorage = class {
	...
	* ```
	*/
	async putFromBase64(dataUrl, location, options) {
	...
	*/
	async copyDirectory(from, to, options) {
		...
		const filesToCopy = (await this.list(from, { recursive: true })).filter((f) => !f.isDirectory);
		let copied = 0;
		for (let i = 0; i < filesToCopy.length; i += concurrency) {
			const batch = filesToCopy.slice(i, i + concurrency);
			await Promise.all(batch.map(async (file) => {
				const newPath = `${to}/${file.path.substring(from.length).replace(/^\//, "")}`;
	...
	* ```
	*/
	async moveDirectory(from, to, options) {
	...
	* ```
	*/
	async putDirectory(localDirPath, destination, options) {
		const concurrency = options?.concurrency ?? 5;
		const localFiles = await this.walkLocalDirectory(localDirPath);
		const filteredFiles = options?.filter ? localFiles.filter(({ absolute, relative }) => options.filter(absolute, relative)) : localFiles;
		...
		for (let i = 0; i < filteredFiles.length; i += concurrency) {
			...
			await Promise.all(batch.map(async ({ absolute, relative }) => {
	...
	* @internal
	*/
	async walkLocalDirectory(dirPath, baseDir) {
	...
	*/
	async emptyDirectory(path) {
		const filePaths = (await this.list(path, { recursive: true })).filter((f) => !f.isDirectory).map((f) => f.path);
	...
	*/
	async toBuffer(file) {
		...
		if (this.isReadable(file)) return this.streamToBuffer(file);
		if (typeof file === "string") {
			if (await fileExistsAsync(file)) return fs$1.readFile(file);
	...
	*/
	append(location, suffix) {
		...
		if (lastDot === -1) return `${location}${suffix}`;
...
//# sourceMappingURL=scoped-storage.mjs.map