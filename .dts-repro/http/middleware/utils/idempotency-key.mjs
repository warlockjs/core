...
//#region ../../@warlock.js/core/src/http/middleware/utils/idempotency-key.ts
/**
...
function isValidIdempotencyKey(value) {
	return typeof value === "string" && value.length > 0 && value.length <= MAX_KEY_LENGTH && PRINTABLE_ASCII.test(value);
}
/**
...
function buildIdempotencyCacheKey(request, idempotencyKey) {
	return `idem:${request.decodedAccessToken?.userType || "anonymous"}:${request.user?.id || request.detectIp() || "unknown"}:${idempotencyKey}`;
}
//#endregion
export { buildIdempotencyCacheKey, hashBody, isValidIdempotencyKey };

//# sourceMappingURL=idempotency-key.mjs.map