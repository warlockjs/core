import { BenchmarkSnapshots } from "./benchmark-snapshots.mjs";
import { BenchmarkProfiler } from "./profiler.mjs";

//#region ../../@warlock.js/core/src/benchmark/types.d.ts
/**
...
type BenchmarkOptions<T = unknown> = {
  /**
 ...
 */
type BenchmarkSuccessResult<T> = BenchmarkResult & {
  success: true;
  value: T;
};
/**
 ...
 */
type BenchmarkErrorResult = BenchmarkResult & {
  success: false;
  error: unknown;
};
...
type BenchmarkStats = {
  ...
  avg: number;
  min: number;
  max: number; /** Total call count — unbounded, not capped by ring buffer */
  count: number; /** Total error count — unbounded */
  errors: number; /** 0.0–1.0 — based on unbounded total/error counters */
  errorRate: number;
  firstSeenAt: Date;
  lastSeenAt: Date;
};
type BenchmarkProfilerOptions = {
  /** Max latency samples to keep per operation name. Default: 1000 */maxSamples?: number; /** Channels to broadcast stats to on flush(). Default: [new NoopChannel()] */
  channels?: BenchmarkChannel[]; /** Auto-flush interval in ms. If not set, flush() must be called manually. */
  flushEvery?: number;
};
type BenchmarkSnapshotsOptions = {
  /** Max snapshots to keep per operation name. Default: 100 */maxSnapshots?: number;
  /**
   * What to capture:
...
//#endregion
export { BenchmarkChannel, BenchmarkConfigurations, BenchmarkErrorResult, BenchmarkOptions, BenchmarkProfilerOptions, BenchmarkResult, BenchmarkSnapshotsOptions, BenchmarkStats, BenchmarkSuccessResult };
